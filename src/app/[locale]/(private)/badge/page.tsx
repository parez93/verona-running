...
export default function AchievementsPage() {
    ...
    // 🔹 Fetch user and achievements
    useEffect(() => {
        const loadData = async () => {
            const currentUser: User = await userByIdAct();
            ...
            if (!achiev.ok) {
                throw new Error('Errore nel caricamento dei badge');
            }
    ...
    // 🔹 Calcolo punti totali e livelli
    const userPoints = unlockedAchievements.reduce((sum, a) => sum + (a.points || 0), 0);
    ...
    const levels = [
        ...
        { level: 4, minPoints: 80, maxPoints: 159, icon: '🔥', color: 'from-orange-400 to-orange-600' },
        { level: 5, minPoints: 160, maxPoints: 319, icon: '🏆', color: 'from-yellow-400 to-yellow-600' },
        { level: 6, minPoints: 320, maxPoints: Infinity, icon: '👑', color: 'from-cyan-400 to-purple-600' },
    ];
            ...
            <div className="container mx-auto px-4 py-8 max-w-7xl">
                ...
                <div className="mb-8">
                    ...
                    >
                        <Card className="p-8 bg-gradient-to-br from-card to-card/50 border-2 border-primary/20">
                            <div className="flex flex-col md:flex-row items-center gap-8">
                                ...
                                >
                                    ...
                                    >
                                        <Avatar
                                            className={`AvatarRoot w-28 h-28 rounded-full overflow-hidden bg-gradient-to-br ${currentLevel.color}`}
                                        >
                                            <AvatarImage
                                                src={user?.img_base64 || undefined}
                                                alt={user?.email || 'User'}
                                            />
                                            <AvatarFallback
                                    ...
                                    >
                                        <div className="h-5 w-5 text-center text-primary-foreground">
                                            {currentLevel.icon}
                                        </div>
                                    </motion.div>
                                </motion.div>
                                    ...
                                    >
                                        ...
                                        {nextLevel ? (
                                            <p className="text-sm text-muted-foreground">
                                                Livello {currentLevel.level} • {userPoints.toLocaleString('it-IT')} punti
                                            </p>
                                        ) : (
                        ...
                        <span className="font-semibold">
                          Progresso verso Livello {nextLevel.level}
                        </span>
                        ...
                        <span>
                          Ti mancano{' '}
                            <span className="font-semibold text-foreground">{pointsToNext}</span> punti al
                          prossimo livello
                        </span>
                ...
                <Tabs value={filter} onValueChange={(v) => setFilter(v as any)} className="mb-6">
                    <TabsList className="grid w-full max-w-md mx-auto grid-cols-3">
                        <TabsTrigger value="all">Tutti ({achievements.length})</TabsTrigger>
                        ...
                        >
                            <Card
                                className={`p-6 cursor-pointer transition-all ${
                                    achievement.unlocked ? 'hover:shadow-lg' : 'opacity-60 hover:opacity-80'
                                }`}
                            ...
                            >
                                ...
                                <div className="flex items-center justify-center mb-3">
                                    <Badge
                                        variant="outline"
                                        className={`capitalize bg-gradient-to-r ${getTierColor(
                                            achievement.tier
                                        )} text-white border-0`}
                                    >
                                        {achievement.tier}
