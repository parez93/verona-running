...
import { Badge, Trophy, Calendar, Target, TrendingUp, Award } from "lucide-react"
...
export function NotificationItem_tmp({ notification, onMarkAsRead, onClose }: NotificationItemProps) {
      ...
      `}
        >
            ...
            <div className="flex-shrink-0 mt-1">
                <div className={`
          p-2 rounded-full
          ${notification.type === "badge" || notification.type === "achievement" ? "bg-[#e67e22]/10" : "bg-[#1e3a5f]/10"}
        `}>
                    {getIcon()}
                </div>
            </div>
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                    <h4 className={`text-sm font-semibold line-clamp-1 ${!notification.read ? "text-foreground" : "text-muted-foreground"}`}>
                        {notification.title}
                    </h4>
                    {!notification.read && (
