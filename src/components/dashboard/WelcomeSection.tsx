...
export default function WelcomeSection({user, stats}: WelcomeSectionProps) {
    const t = useTranslations("dashboard.welcomeSection"); // ✅ hook traduzioni

    return (
        <section
            className="h-full overflow-hidden border-border/60 rounded-[var(--radius)] border bg-gradient-to-br from-blue-600 via-blue-500 to-purple-600  text-card-foreground shadow-sm ">
            {/*
...
*/}
            <div className="pl-8 pe-4 text-white border-0  relative overflow-hidden pt-8">
                ...
                {/* Stats Grid */}
                <div
                    className="grid grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-4 sm:gap-6 relative z-10 pr-20 sm:pr-24 md:pr-0">
                    <div className="space-y-1 sm:space-y-2">
                        <p className="text-white/80 text-xs sm:text-sm md:text-base">{t("eventsRun")}</p>
                        <p className="text-2xl sm:text-3xl md:text-4xl font-bold">{stats.eventsRun}</p>
                    </div>
                    <div className="space-y-1 sm:space-y-2 pl-10">
                        <p className="text-white/80 text-xs sm:text-sm md:text-base">{t("kmRun")}</p>
                        <p className="text-2xl sm:text-3xl md:text-4xl font-bold">{stats.kmRun}</p>
                    </div>
                </div>
